...
$$('#avatar2').on('click', function () {
    var buttons = [
        {
            text: '<i class="f7-icons">camera</i>',
            bold: false,
            onClick: function () {
               capturePhotoEdit();
        ...
        {
            text: '<i class="f7-icons">images</i>',
            bold: false,
            onClick: function () {
...
/* @TODO FizzQuizzAWS credentials */
// Setup your App  ID and API key
var applicationId = 'xxx';
var restApiKey = 'yyy';
var alertTitle = 'Fountain KO';
...
// Connection check if app is connected server
...
function check_storage() {
    ...
    if (localStorage['userlogin']) {
        ...
        $$('.logout-button').show();
    		$$('.left a').show();
    		$$('.right').show();
        $$('.notlogged.toolbar-inner').hide();
        $$('.common.toolbar-inner').css('display','inline-flex');
        console.log('logged');



  //  mainView.router.loadPage('#welcome');
//  $$('common.toolbar-inner').show();
...
function myProfile() {
   ...
   $.getJSON(base_url + '/get_user_details/' + user, function(result) {
        $.each(result, function(i, field) {
            ...
            $('.profile-image').css('background-image', 'url(' + profile_photo + ')');
            $('.profile-image').css('background-size', 'cover');
            $('.profile-image').css('background-position', 'center center');
        ...
        var comment_count = field.comment_count;
			...
			$(".oldposts").append('<li>'+
                              '<a class="item-link post-link item-content" id="'+frompost_id+'"  href="'+base_url+'/single/post/'+frompost_id+'">'+
              									'<img src="'+thumbnail+'" width="80" style="margin-right:10px; width:50px;"></div>'+
...
function imageProfile() {
    ...
    function showResponse(responseText, statusText, xhr, $form) {
        ...
        if (statusText == 'success') {
            ...
            var profilePhoto = $('#smallImage').attr('src');
            $('#upload_input').val(profilePhoto);
...
function log_out() {
    ...
    $$('#loginScreen').removeClass('modal-out');
...
//  var comment_date = displayDate+''+displayTime;
  var timeInMs = Date.now();
  var comment_date = timeInMs;
